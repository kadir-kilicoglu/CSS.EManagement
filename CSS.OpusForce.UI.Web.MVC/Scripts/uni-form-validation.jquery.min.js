...
$input.css('color', $input.data('default-color')); return;
}); form.delegate(settings.field_selector, 'error', function (e, text) { validate($(this), false, text); }); form.delegate(settings.field_selector, 'success', function (e, text) { validate($(this), true); }); $('input[autofocus]:first').focus();
});
}; jQuery.fn.uniform.showFormError = function (form, title, messages) {
    var m, $message; if ($('.validation-summary-errors').length) { $('.validation-summary-errors').remove(); }
    $message = $('<div />').attr('id', 'validation-summary-errors').html("<h3>" + title + "</h3>"); if (messages.length) { $message.append($('<ol />')); for (m in messages) { $('ol', $message).append($('<li />').text(messages[m])); } }
form.prepend($message); $('html, body').animate({ scrollTop: form.offset().top }, 500); $('.validation-summary-errors').slideDown(); return false;
}; jQuery.fn.uniform.showFormSuccess = function (form, title) {
    var $message; if ($('#okMsg').length) { $('#okMsg').remove(); }
